...
<script src="'.RES.'/css/bjdh/'.$this->homeInfo['bjdh'].'/bjdh'.$this->homeInfo['bjdh'].'.js" type="text/javascript"></script>');
	...
	public function lists(){
		...
		$res=$db->where($where)->order('sorts DESC')->limit("{$p},".$pageSize)->select();
		...
		if($ifone)$this->redirect(U('Index/index',array('classid'=>$this->_get('classid'),'token'=>$this->token,'tpl'=>$l['tpl'])));
