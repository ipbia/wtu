...
if ($emailstatus == 1) {
    ...
    if ($content) {
        ...
        $mail->SetFrom($emailuser.'@qq.com', '微信系统');

        $mail->AddReplyTo($emailuser.'@qq.com', '微信系统');
...
// 结束
	...
	public function sms(){
		...
		if ($orders){
			...
			$str.="合计：<font color='#ff0000'>".$thisOrder['price']."</font>元<div style='color:#666;border-top:1px solid #ccc;padding订单类型：".$orderType."\r\n订单编号：".$thisOrder['id']."\r\n姓名：".$thisOrder['truename']."\r\n电话：".$thisOrder['tel']."\r\n地址：".$thisOrder['address']."\r\n合计：".$thisOrder['price']."元\r\n
