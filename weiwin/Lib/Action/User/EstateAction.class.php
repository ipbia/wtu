...
class EstateAction extends UserAction
{
    ...
    public function son_del()
    {
        ...
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token,'thetype' => 'Estate');
        $check = $t_son->where($where)->find();
        if ($check == null) {
            $this->error('操作失败');
        } else {
    ...
    public function housetype_add()
    {
        ...
        $son_data = M('Estate_son')->where(array('token' => session('token'),'thetype' => 'Estate'))->field('id as sid,title')->select();
        $this->assign('son_data', $son_data);
    ...
    public function album_del()
    {
        ...
        if ($check == null) {
            ...
            $isok = $housetype->where($where)->delete();
            if ($isok) {
                $this->success('删除成功', U('Estate/album', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Estate/album', array('token' => session('token'))));
            }
        }
    }
