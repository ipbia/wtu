...
class UpyunAction extends UserAction{
	...
	function localUpload($filetypes=''){
		...
		$upload->savePath =  $path.'/uploads/'.$firstLetter.'/'.$this->token.'/';// 设置附件上传目录
		//
		if (!file_exists($path.'/uploads')||!is_dir($path.'/uploads')){
			mkdir($path.'/uploads',0777);
		}
		$firstLetterDir=$path.'/uploads/'.$firstLetter;
		...
		if(!$upload->upload()) {// 上传错误提示错误信息
			...
			$upload->savePath = str_replace($path, '', $upload->savePath);
			if ($thumb==1){
				$paths=explode('/',$info[0]['savename']);
				$fileName=$paths[count($paths)-1];
				$msg=C('site_url').substr($upload->savePath,0).str_replace($fileName,'thumb_'.$fileName,$info[0]['savename']);
			}else {
				$msg=C('site_url').substr($upload->savePath,0).$info[0]['savename'];
			}
