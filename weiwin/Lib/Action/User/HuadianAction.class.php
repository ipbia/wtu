...
class HuadianAction extends UserAction
{
        ...
        if (IS_POST) {
            if ($es_data == null) {
                if ($id = $data->add($_POST)) {
                    $data1['pid'] = $id;
                    $data1['module'] = 'Huadian';
                    $data1['token'] = session('token');
                    $data1['keyword'] = trim($_POST['keyword']);
                    M('Keyword')->add($data1);
                    $this->success('添加成功', U('Huadian/index', array('token' => session('token'))));
                    die;
                } else {
                ...
                if ($data->where($wh)->save($_POST)) {
                    ...
                    $this->success('修改成功', U('Huadian/index', array('token' => session('token'))));
                } else {
                    $this->error('操作失败');
                }
            }
    ...
    public function son_add()
    {
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_son->add($_POST)) {
                    $this->success('添加成功', U('Huadian/son', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_son->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Huadian/son', array('token' => session('token'))));
                } else {
                    $this->error('操作失败');
                }
            }
    ...
    public function son_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Huadian/son', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Huadian/son', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_housetype->add($_POST)) {
                    $this->success('添加成功', U('Huadian/housetype', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_housetype->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Huadian/housetype', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function housetype_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Huadian/housetype', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Huadian/housetype', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            if ($check == NULL) {
                ...
                if ($t_album->add($_POST)) {
                    $this->success('添加成功', U('Huadian/album', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_album->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Huadian/album', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function album_del()
    {
        ...
        if ($check == null) {
            ...
            $isok = $housetype->where($where)->delete();
            if ($isok) {
                $this->success('删除成功', U('Huadian/album', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Huadian/album', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            if ($check == null) {
                if ($t_impress->add($_POST)) {
                    $this->success('添加成功', U('Huadian/impress', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_impress->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Huadian/impress', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function impress_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Huadian/impress', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Huadian/impress', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            if ($check == null) {
                if ($t_expert->add($_POST)) {
                    $this->success('添加成功', U('Huadian/expert', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_expert->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Huadian/expert', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function expert_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Huadian/expert', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Huadian/expert', array('token' => session('token'))));
            }
        }
    }
