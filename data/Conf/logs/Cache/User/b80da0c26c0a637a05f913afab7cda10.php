...
<h4 class="left">订单列表 (<?php echo ($page); ?>) </h4>

<a href="<?php echo U($type.'/index',array('token'=>$token));?>" class="right  btnGreen" >返回</a>
...
<div class="msgWrap">
	...
	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><form method="post" action="" id="info">
		...
		<td>
			...
			<a href="<?php echo U($type.'/setType',array('id'=>$item['id'],'pid'=>$item['pid'],'token'=>$token, 'type'=>0 ));?>">

			<input type="radio" name="type" <?php if($item['type'] == 0 ) echo 'checked="checked"';?> /><span style="background:#999; color:#fff;">等待</span>

			</a>

			<a href="<?php echo U($type.'/setType',array('id'=>$item['id'],'pid'=>$item['pid'],'token'=>$token, 'type'=>1 ));?>">

			<input type="radio" name="type" <?php if($item['type'] == 1 ) echo 'checked="checked"';?> /><span style="background:#360; color:#fff;">确定</span>

			</a>

			<a href="<?php echo U($type.'/setType',array('id'=>$item['id'],'pid'=>$item['pid'],'token'=>$token, 'type'=>2 ));?>">

			<input type="radio" name="type" <?php if($item['type'] == 2 ) echo 'checked="checked"';?> /><span style="background:red; color:#fff;">拒绝</span>
		...
		<td class="norightborder">
			...
			<a href="<?php echo U($type.'/infos_detail',array('id'=>$item['id'],'token'=>$token));?>">查看详细信息 </a>&nbsp;&nbsp;

			<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U($type.'/delinfos',array('id'=>$item['id'],'token'=>$token));?>');">删除 </a>
