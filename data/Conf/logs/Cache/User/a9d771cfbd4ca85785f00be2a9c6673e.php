...
<li class="<?php if(ACTION_NAME == 'huadianindex'): ?>current<?php endif; ?> tabli" id="tab6"><a href="<?php echo U('Reservation/huadianindex',array('token'=>$token));?>">预约管理</a></li>
          ...
          <div class="cLine">
    <div class="alert">
    <p><span class="bold">使用方法：</span>请添加点评</p>
    </div>
</div>
          
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Huadian/expert_add',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
           ...
           <input name="wxid" type="hidden" value="gh_423dwjkewad">
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              <thead>
                                       ...
                                       <tbody>
                                            <?php if($expert == ''): ?><tr>
                                        <td colspan="5" style="text-align:center; height:30px;"><strong>请加添客人印象信息.</strong></td>
                                        </tr>
                                    <?php else: ?>
                                    <?php if(is_array($expert)): $i = 0; $__LIST__ = $expert;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                         
                                        <td><?php echo ($list['title']); ?></td>
                                        <td><?php echo ($list['name']); ?></td> 
                                        <td><?php echo ($list['sort']); ?></td>
                                        <td>
                                        <a href="<?php echo U('Huadian/expert_add',array('id'=>$list['id'],'token'=>$list['token']));?>" class="btn">编辑</a>
                                        <a href="<?php echo U('Huadian/expert_del',array('id'=>$list['id'],'token'=>$list['token']));?>" onclick="return(confirm('确定要删除吗？'))"  class="btn">删除</a></a>
</td>
