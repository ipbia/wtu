...
<th style=" width:100px;">父级类别<a target="999" class="tooltips" ><img src="<?php echo RES;?>/images/price_help.png" align="absmiddle" /><span>

<p>父级类别是之本分分类隶属于的上级类别，如果是根分类，代表属于主分类！</p>
...
<th>外链<span class="tooltips" ><img src="<?php echo RES;?>/images/price_help.png" align="absmiddle" /><span>

<p>比如你想跳转到百度，就填写<a class="red">http://www.baidu.com</a></p>

<p>不需要跳转就不用填写，还可以外链活动，</p>

<p>直接填写活动外链地址：<a class="red">大转盘</a></p>

</span></span></th>

<th style=" width:60px;">显示</th>

                        <th style=" width:120px;" class="norightborder">操作</th>
...
<td align="center"><span style="font-size:32px;color:#999999;"><em><strong><?php echo ($vo["sorts"]); ?></strong></em></span></td>

<td align="center"><?php echo ($vo["name"]); if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zo): $mod = ($i % 2 );++$i; if($zo['id'] == $vo['fid']): ?><br><a class="red">属于子分类</a><?php endif; endforeach; endif; else: echo "" ;endif; ?></td>

<td align="center"><?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zo): $mod = ($i % 2 );++$i; if($zo['id'] == $vo['fid']): echo ($zo["name"]); endif; endforeach; endif; else: echo "" ;endif; ?>

</td>

<td bgcolor="<?php echo ($vo["yanse"]); ?>"><span class="<?php echo ($vo["icon"]); ?>"></span></td>
...
<td align="center"><?php if(($vo["status"]) == "1"): ?>显示<?php else: ?>不显示<?php endif; ?><br><a target="ddd" href="<?php echo U('Wap/Index/classify',array('token'=>$_SESSION['token']));?>">查看</a></td>

<td align="center" class="norightborder">

	<a style="color:blue" href="<?php if ($vo['fid']==0){ echo U('Classify/edit',array('id'=>$vo['id'])); }else{ echo U('Classify/edit',array('id'=>$vo['id'],'fid'=>$vo['fid'])); }?>">编辑</a>&nbsp; &nbsp; &nbsp;

	<a style="color:red" href="javascript:drop_confirm('您确定要删除吗?', '<?php if ($vo['fid']==0){ echo U('Classify/del',array('id'=>$vo['id'])); }else{ echo U('Classify/del',array('id'=>$vo['id'],'fid'=>$vo['fid'])); }?>');">删除</a>
