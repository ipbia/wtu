...
<div class="cLineB">
  ...
  <h4>微信墙活动设置<span class="FAQ">每次只能开启一个活动，上个活动未结束不能创建下个活动</span></h4>
 ...
 <a href="<?php echo U('Wewall/add');?>" class="btnGrayS vm bigbtn" >添加活动</a>　 
...
<th style=" width:110px;">前台发送间隔</th>
...
<td><?php echo ($actinfo["showtime"]); ?>秒</td>



<td><?php if($actinfo['ifcheck'] == '1'): ?>需要审核<?php else: ?>不需要审核<?php endif; ?></td>



<td><?php if($actinfo['iflottery'] == '1'): ?>已开启抽奖<?php else: ?>未开启抽奖<?php endif; ?></td>



<td><?php if($actinfo['background'] == Null): ?>未设置自定义背景<?php else: ?><img style="width:100px" src="<?php echo ($info["background"]); ?>"><?php endif; ?></td>
...
<td class="norightborder">
    ...
    <?php if($actinfo['isact'] == 0): ?><a href="<?php echo U('Wewall/change',array('id'=>$actinfo['id'],'actid'=>0,'token'=>$_SESSION['token']));?>">启动</a>



    <?php else: ?> <a href="<?php echo U('Wewall/change',array('id'=>$actinfo['id'],'actid'=>1,'token'=>$_SESSION['token']));?>">停止</a><?php endif; ?>



    <a target="_blank" href="<?php echo U('Wewall/show',array('token'=>$_SESSION['token'],'id'=>$actinfo['id']));?>">前台</a>



    <?php if($actinfo['ifcheck'] == 1): ?><a target="_blank" href="<?php echo U('Wewall/gocheck',array('token'=>$_SESSION['token'],'id'=>$actinfo['id']));?>">审核</a><?php endif; ?>



    <a href="<?php echo U('Wewall/edit',array('id'=>$actinfo['id'],'token'=>$_SESSION['token']));?>" >编辑</a>



    <a href="<?php echo U('Wewall/change',array('id'=>$actinfo['id'],'actid'=>2,'token'=>$_SESSION['token']));?>" title="统计活动结果并关闭游戏，无法再次开启">关闭</a>
...
<td class="norightborder">
	...
	<a target="ddd" href="<?php echo U('Wewall/result',array('id'=>$vo['id'],'token'=>$_SESSION['token']));?>">查看结果</a>



	<a href="<?php echo U('Wewall/delold',array('id'=>$vo['id']));?>">删除</a>
